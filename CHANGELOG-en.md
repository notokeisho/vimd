...
- **Enhanced LaTeX preview**
  - Added `\bm` (bold math) macro support
  - Added MathJax equation numbering (`tags: 'ams'`)
  - `\label{}`/`\ref{}` cross-references now work correctly
  - Auto-extract and display title, Abstract, and Key Points from LaTeX files
  - Added CSS styles for LaTeX metadata to all themes

### Fixed

- **LaTeX command display issues** (Issue #10)
  - Fixed `\bm` and other special commands not rendering
  - Fixed equation numbers showing as `(???)`
